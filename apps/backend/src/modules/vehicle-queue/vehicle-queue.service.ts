...
export class VehicleQueueService {
  ...
  async list(tenantId: string, outletId: string, includeDone = false): Promise<QueueEntry[]> {
    const statusFilter = includeDone ? '' : `AND status IN ('waiting','serving')`;
    const res = await this.pool.query(
      `SELECT * FROM vehicle_queue WHERE tenant_id = $1 AND outlet_id = $2 ${statusFilter}
       ORDER BY (status = 'done') ASC, position ASC, created_at ASC LIMIT 200`,
      [tenantId, outletId],
    );
