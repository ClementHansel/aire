...
export interface CategoryRecord {
  id: string; tenantId: string; name: string; sortOrder: number; isActive: boolean;
}
export interface BrandRecord {
  id: string; tenantId: string; code: string; name: string; color: string; isActive: boolean;
}
...
export class CatalogService {
  ...
  async createCategory(tenantId: string, name: string, sortOrder = 0): Promise<CategoryRecord> {
    if (!name?.trim()) throw new BadRequestException('name is required');
    const res = await this.pool.query(
      `INSERT INTO product_categories (tenant_id, name, sort_order) VALUES ($1, $2, $3) RETURNING *`,
      [tenantId, name.trim(), sortOrder],
    );
    return this.mapCategory(res.rows[0]);
  }

  async updateCategory(tenantId: string, id: string, patch: { name?: string; sortOrder?: number; isActive?: boolean }): Promise<CategoryRecord> {
  ...
  async createBrand(tenantId: string, dto: { code: string; name: string; color?: string }): Promise<BrandRecord> {
    if (!dto.code?.trim() || !dto.name?.trim()) throw new BadRequestException('code and name are required');
    const res = await this.pool.query(
      `INSERT INTO brands (tenant_id, code, name, color) VALUES ($1, $2, $3, $4) RETURNING *`,
      [tenantId, dto.code.trim().toUpperCase(), dto.name.trim(), dto.color ?? '#1652F0'],
    );
    return this.mapBrand(res.rows[0]);
  }

  async updateBrand(tenantId: string, id: string, patch: { name?: string; color?: string; isActive?: boolean }): Promise<BrandRecord> {
    const set: string[] = []; const v: unknown[] = []; let i = 1;
    if (patch.name !== undefined) { set.push(`name = $${i++}`); v.push(patch.name); }
  ...
  private mapCategory = (r: any): CategoryRecord => ({
    id: r.id, tenantId: r.tenant_id, name: r.name, sortOrder: r.sort_order ?? 0, isActive: r.is_active,
  });
  private mapBrand = (r: any): BrandRecord => ({
    id: r.id, tenantId: r.tenant_id, code: r.code, name: r.name, color: r.color ?? '#1652F0', isActive: r.is_active,
  });
}
