...
describe('AdminService', () => {
  ...
  describe('suspendTenant', () => {
    it('should suspend an active tenant', async () => {
      ...
      const result = await service.suspendTenant('tenant-001');
    ...
    it('should throw NotFoundException when tenant does not exist', async () => {
      mockPool.query
        ...
        .mockResolvedValueOnce({ rows: [] }); // existence check also empty
    ...
    it('should throw BadRequestException when tenant is not active', async () => {
      mockPool.query
        .mockResolvedValueOnce({ rows: [] }) // update returns nothing (not active)
        .mockResolvedValueOnce({ rows: [{ status: 'cancelled' }] }); // exists but cancelled

      await expect(service.suspendTenant('tenant-001')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('reactivateTenant', () => {
    it('should reactivate a suspended tenant', async () => {
      ...
      const result = await service.reactivateTenant('tenant-001');
    ...
    it('should throw NotFoundException when tenant does not exist', async () => {
      ...
      await expect(service.reactivateTenant('nonexistent')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw BadRequestException when tenant is not suspended', async () => {
      ...
      await expect(service.reactivateTenant('tenant-001')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
