...
export class MembershipAdminService {
  ...
  async list(tenantId: string, statusFilter?: string): Promise<MembershipListRow[]> {
    const res = await this.pool.query(
      `SELECT m.id, c.name AS customer_name, c.phone AS customer_phone, mp.name AS plan_name,
              ...
              CASE
                ...
                WHEN m.status IN ('cancelled','pending') THEN m.status
  ...
  async suspend(tenantId: string, id: string, reason?: string): Promise<void> {
    const res = await this.pool.query(
