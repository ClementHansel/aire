...
export default function TransactionsPage() {
  ...
  const computeInsights = (s: SeriesPoint[], sum: Summary) => {
    ...
    if (s.length >= 2) {
      ...
      if (first > 0) {
        ...
        out.push(`Revenue ${delta >= 0 ? 'grew' : 'declined'} ${Math.abs(delta)}% in the second half of the range vs the first.`);
      }
    }
    const best = [...s].sort((a, b) => b.revenue - a.revenue)[0];
    if (best) out.push(`Best ${granularity}: ${best.period} with ${fmt(best.revenue)} across ${best.orders} orders.`);
    if (sum.byService[0]) out.push(`Top product: ${sum.byService[0].name} (${sum.byService[0].quantity} sold, ${fmt(sum.byService[0].revenue)}).`);
    const aire = sum.byBusinessUnit?.AIRE?.revenue ?? 0; const lead = sum.byBusinessUnit?.LEAD?.revenue ?? 0;
    if (aire + lead > 0) out.push(`Business mix: AIRE ${Math.round((aire / (aire + lead)) * 100)}% / LEAD ${Math.round((lead / (aire + lead)) * 100)}%.`);
    if (sum.cancelledCount > 0) out.push(`${sum.cancelledCount} cancelled order(s) in range — review for voids/errors.`);
    out.push(`Total revenue in range: ${fmt(totalRev)} from ${sum.paidCount} paid orders.`);
    setInsights(out);
  };
  ...
  const exportExcel = () => {
    ...
    const html = `<table border="1"><thead><tr><th>Order</th><th>Date</th><th>Customer</th><th>Phone</th><th>Status</th><th>Total</th></tr></thead><tbody>${rows}</tbody></table>`;
  ...
  const exportPdf = () => {
    ...
    w.document.write(`<!doctype html><html><head><title>Executive Report</title><style>
      ...
      <h1>AIRE — Executive Report</h1>
      <p class="muted">${dateFrom} → ${dateTo}${businessUnit ? ` · ${businessUnit}` : ''}</p>
      <div class="kpis">
        <div class="kpi"><div class="muted">Revenue</div><div class="v blue">${fmt(summary.revenue)}</div></div>
        <div class="kpi"><div class="muted">Paid orders</div><div class="v">${summary.paidCount}</div></div>
      ...
      <h3>Insights</h3><ul>${insHtml}</ul>
      <h3>Top products</h3><table><thead><tr><th>Product</th><th style="text-align:right">Qty</th><th style="text-align:right">Revenue</th></tr></thead><tbody>${svcRows}</tbody></table>
      <p class="muted" style="margin-top:40px">Generated ${new Date().toLocaleString()} · AIRE Operations</p>
      <script>window.onload=()=>window.print()</script></body></html>`);
    w.document.close();
  };

  const del = async (o: OrderCard) => {
    if (!confirm(`Delete/void order ${o.orderNumber}?`)) return;
    ...
    <div data-testid="transactions-page">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">Revenue & product charts, transaction table, AI insights, and exports.</p>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="card"><p className="text-xs text-text-muted uppercase">Revenue</p><p className="text-2xl font-bold text-primary-600 mt-1">{fmt(summary.revenue)}</p></div>
          <div className="card"><p className="text-xs text-text-muted uppercase">Paid orders</p><p className="text-2xl font-bold mt-1">{summary.paidCount}</p></div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        <div className="card"><h2 className="section-title mb-3">Revenue ({granularity})</h2><Bars data={revenueBars} /></div>
        <div className="card"><h2 className="section-title mb-3">Sales per product</h2><Bars data={productBars} /></div>
      </div>
      ...
      <div className="card p-0 overflow-hidden">
        ...
        <table className="w-full">
          <thead><tr className="border-b border-border bg-surface-sunken/50">
            <th className="text-left px-5 py-3 text-xs font-medium text-text-secondary uppercase">Order</th>
          ...
          <tbody className="divide-y divide-border">
            {orders.length === 0 ? <tr><td colSpan={5} className="px-5 py-6 text-sm text-text-muted text-center">No transactions.</td></tr> : orders.map((o) => (
              <tr key={o.id}>
                ...
                <td className="px-5 py-3 text-right whitespace-nowrap">
                  <button className="btn-ghost text-xs" onClick={() => setDetail(o)}>View</button>
                  <button className="btn-ghost text-xs" onClick={() => setEditing(o)}>Edit</button>
                  {o.status !== 'cancelled' && <button className="btn-ghost text-xs text-red-600" onClick={() => del(o)}>Delete</button>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setDetail(null)}>
          <div className="card w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <h3 className="section-title mb-3">Order {detail.orderNumber}</h3>
            <div className="text-sm space-y-1.5">
              <div className="flex justify-between"><span className="text-text-muted">Customer</span><span>{detail.customerName}</span></div>
              <div className="flex justify-between"><span className="text-text-muted">Phone</span><span>{detail.customerPhone}</span></div>
              <div className="flex justify-between"><span className="text-text-muted">Operator</span><span>{detail.operatorName}</span></div>
              <div className="flex justify-between"><span className="text-text-muted">Status</span><span className="capitalize">{detail.status}</span></div>
              <div className="flex justify-between"><span className="text-text-muted">Total</span><span className="font-medium">{fmt(detail.total)}</span></div>
              <div className="flex justify-between"><span className="text-text-muted">Date</span><span>{new Date(detail.createdAt).toLocaleString()}</span></div>
            </div>
            <button className="btn-secondary w-full mt-4" onClick={() => setDetail(null)}>Close</button>
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="card w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 justify-end mt-4">
          ...
          <button className="btn-primary" onClick={() => onSave({ customerName, customerPhone })}>Save</button>
        </div>
      </div>
    </div>
  );
