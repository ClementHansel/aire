...
interface Schedule { id: string; employee: string; workDate: string; startTime: string | null; endTime: string | null; }
...
export default function HrPage() {
  ...
  const [schedForm, setSchedForm] = useState({ employeeId: '', workDate: '', startTime: '', endTime: '' });
  ...
  const load = useCallback(async () => {
    try {
      const [s, e, l, sch, h] = await Promise.all([
        api.get<Summary>('/hr/summary'),
        api.get<Employee[]>('/hr/employees'),
        api.get<Leave[]>('/hr/leave'),
        api.get<Schedule[]>('/hr/schedules'),
        api.get<Holiday[]>('/hr/holidays'),
      ]);
      setSummary(s); setEmployees(e); setLeave(l); setSchedules(sch); setHolidays(h); setError('');
  ...
  const addEmployee = () => empForm.name.trim() && wrap(async () => {
    await api.post('/hr/employees', { name: empForm.name.trim(), role: empForm.role || undefined, phone: empForm.phone || undefined, salary: Number(empForm.salary) || 0 });
  ...
  const setSchedule = () => schedForm.employeeId && schedForm.workDate && wrap(async () => {
    await api.post('/hr/schedules', { employeeId: schedForm.employeeId, workDate: schedForm.workDate, startTime: schedForm.startTime || undefined, endTime: schedForm.endTime || undefined });
    setSchedForm({ employeeId: '', workDate: '', startTime: '', endTime: '' });
  });
            ...
            <div className="space-y-1.5 max-h-96 overflow-auto">
              {employees.map((e) => (
                <div key={e.id} className="flex items-center justify-between text-sm border-b border-border py-2">
                  <span className="text-text-primary">{e.name}<span className="text-text-muted">{e.role ? ` · ${e.role}` : ''} · {fmt(e.salary)}</span></span>
            ...
            <div className="space-y-1.5 max-h-96 overflow-auto">
              {schedules.map((s) => (
                <div key={s.id} className="flex justify-between text-sm border-b border-border py-2">
                  <span className="text-text-primary">{s.employee}</span>
